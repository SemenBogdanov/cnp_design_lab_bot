...
async def main_client(message: types.Message, state: FSMContext):
    ...
    await Projects.next()  # next state: short_task_description
...
async def get_task_description(message: types.Message, state: FSMContext):
    ...
    await Projects.next()  # next state: deadline
...
async def get_deadline_date(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        data['score_importance'] = '--исключено--'
        data['score_urgency'] = '--исключено--'
    await Projects.next()  # next state: timestamp
    async with state.proxy() as data:
        data['timestamp'] = str(datetime.now() + timedelta(hours=3))
        data['task_status'] = 'New!'

    await state.finish()
    await recs(data, message)


async def recs(data, message: types.Message):
    # async with state.proxy() as data:
    ...
    try:
        ...
        if rec_to_sheets(data):
            ...
            # await bot.send_message('-1001613271219', ans, parse_mode='HTML')
        ...
        if bitrix_task_was_created:
            ans = ans + '\n\nПользователь получил уведомление, что с ним свяжутся!'
            ans = ans + f"\n\nСоздана задача в Bitrix24: {bitrix_task_was_created['task']['id']}"

            await bot.send_message('-1001613271219', ans, parse_mode='HTML')
            # await bot.send_message('-1001613271219',
            #                        f"Создана задача в Bitrix24: {bitrix_task_was_created['task']['id']}",
            #                        parse_mode='HTML')
        else:
            await bot.send_message('-1001613271219', "Не удалось создать задачу в Bitrix24")
    except Exception as e:
        await bot.send_message(message.chat.id, "Произошел какой-то сбой. Данные, к сожалению, не записались :( Нужно "
                                                "обязательно позвонить Алексею Анисимову - руководителю "
                                                "дизайн-лаборатории!")
...
def register_chains(dp: Dispatcher):
    ...
    # dp.register_message_handler(recs, state=Projects.timestamp)
